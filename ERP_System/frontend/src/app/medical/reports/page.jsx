...
export default function PharmacyReports() {
  ...
  useEffect(() => {
    // Simulated pharmacy margin analytics
    setData({
      grossProfit: 14250.00,
      costOfGoods: 8550.00,
      netMargins: "40.0%",
      cogsByDrug: [
        { name: "Antibiotics (Amoxicillin)", sales: 4500.00, margin: 45 },
        { name: "Pain Relief (Paracetamol)", sales: 3200.00, margin: 38 },
        { name: "Diabetic Care (Metformin)", sales: 2800.00, margin: 42 }
      ]
    });
  }, []);
    ...
    <div style={{ padding: "24px", fontFamily: "Inter, sans-serif", backgroundColor: "#f8fafc", minHeight: "100vh" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: "24px", marginBottom: "32px" }}>
        ...
        <div style={{ background: "#ffffff", borderRadius: "16px", padding: "24px", border: "1px solid #e2e8f0" }}>
          ...
          <span style={{ fontSize: "28px", fontWeight: "800", color: "#10b981" }}>${(data.grossProfit + data.costOfGoods).toFixed(2)}</span>
        </div>

        <div style={{ background: "#ffffff", borderRadius: "16px", padding: "24px", border: "1px solid #e2e8f0" }}>
          <span style={{ fontSize: "12px", fontWeight: "700", color: "#64748b", display: "block", marginBottom: "8px" }}>COST OF GOODS SOLD (COGS)</span>
          <span style={{ fontSize: "28px", fontWeight: "800", color: "#ef4444" }}>${data.costOfGoods.toFixed(2)}</span>
        </div>
          ...
          {data.cogsByDrug.map(drug => (
            <div key={drug.name}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "14px", fontWeight: "600", marginBottom: "6px" }}>
                <span style={{ color: "#334155" }}>{drug.name} (${drug.sales.toFixed(2)} sales)</span>
