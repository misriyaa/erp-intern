    ...
    try {
      ...
      if (res.success && res.data) {
        const mapped = res.data.map(m => {
          const qty = m.batches?.reduce((sum, b) => sum + b.quantity, 0) || 0;
          return {
            id: m.id,
            sku: m.product?.sku || "N/A",
            name: `${m.product?.name || ""} (${m.genericName})`,
            quantity: qty,
        ...
        if (mapped.length > 0) {
          ...
          setStocks([
            { id: "mock-1", name: "Panadol 500mg (Paracetamol)", sku: "PAN-500", quantity: 800, minStock: 200, unit: "tablets", isMock: true },
            { id: "mock-2", name: "Amoxil 250mg (Amoxicillin)", sku: "AMO-250", quantity: 15, minStock: 50, unit: "capsules", isMock: true },
            { id: "mock-3", name: "Zyrtec 10mg (Cetirizine)", sku: "ZYR-010", quantity: 320, minStock: 100, unit: "tablets", isMock: true }
          ]);
        }
      }
    } catch (err) {
                ...
                {stocks.map(s => (
                  <tr key={s.id} style={{ borderBottom: "1px solid #f1f5f9", fontSize: "14px" }}>
                    ...
                    <td style={{ padding: "12px", fontWeight: "600" }}>
                      {s.name}
                      {s.isMock && <span style={{ marginLeft: "6px", fontSize: "10px", color: "#94a3b8" }}>(Sample)</span>}
                    </td>
